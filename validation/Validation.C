...
void FillPrimaryHistogramCollection(const std::string &histPrefix, const Parameters &parameters, const PrimaryResult &primaryResult, PrimaryHistogramCollection &primaryHistogramCollection)
{
    ...
    if (!primaryHistogramCollection.m_hAngleWithYZEfficiency)
    {
        ...
        primaryHistogramCollection.m_hAngleWithYZEfficiency->GetXaxis()->SetRangeUser(-200., +200.);
    ...
    const int nAngleInYZBins(80); const int nAngleInYZBinEdges(nAngleInYZBins + 1);
    const float angleInYZMin(-200), angleInYZMax(200);
